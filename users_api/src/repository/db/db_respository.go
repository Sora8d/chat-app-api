...
const (
	...
	queryGetUserProfileById = "SELECT up.id, up.user_id, up.active, up.phone, up.first_name, up.last_name, up.username, up.avatar_url, up.description FROM user_profile up JOIN user_table ut on up.user_id = ut.id WHERE ut.uuid=$1;"
	...
	queryInsertUser         = "INSERT INTO user_table default values RETURNING id, uuid;"
	queryUpdateUserProfile  = "UPDATE user_profile as up SET phone=$2, first_name=$3, last_name=$4, username=$5, avatar_url=$6, description=$7 from user_table as ut WHERE up.user_id=ut.id AND ut.uuid = $1 RETURNING up.active, up.phone, up.first_name, up.last_name, up.username, up.avatar_url, up.description;"
	queryUpdateActive       = "UPDATE user_profile up SET up.active=$2 from users_table ut WHERE up.user_id = ut.id and ut.uuid = $1;"
...
func (dbr *userDbRepository) GetUserProfileById(uuid string) (*users.UserProfile, server_message.Svr_message) {
	...
	if err := row.Scan(&profile.Id, &profile.UserId, &profile.Active, &profile.Phone, &profile.FirstName, &profile.LastName, &profile.UserName, &profile.AvatarUrl, &profile.Description); err != nil {
...
func (dbr *userDbRepository) CreateUser(up users.UserProfile) (*string, server_message.Svr_message) {
	client := postgresql.GetSession()
	...
	row_with_user := tx.QueryRow(ctx, queryInsertUser)
	if err := row_with_user.Scan(&newUser.Id, &newUser.Uuid); err != nil {
		transErr := server_message.NewInternalError()
		logger.Error("error trying to create user in CreateUser fuction in the db_repository", err)
		return nil, transErr
	}

	_, err = tx.Exec(ctx, queryInsertUserProfile, newUser.Id, up.Phone, up.Active, up.FirstName, up.LastName, up.Description, up.UserName, up.AvatarUrl)
	if err != nil {
